...
class UI:
    ...
    def view_item(self, n):
        ...
        def change_item(up, current):
            ...
            :return: None
            """
            if 0 <= current < len(self.item_list.items):
                if up:
                    if (current + 1) < len(self.item_list.items):
        ...
        try:
            self._clear_widget(self.img_frame)
        except:
            pass
        try:
            self._clear_widget(self.specifics_frame)
        except:
            pass
    ...
    def show_items(self):
        ...
        self.frames_list = []
        self.scroll_height = len(self.item_list.items) * 42
    ...
    def get_options(self, upload_obj, start=""):
        ...
        self.end_point_var = tk.StringVar(value="")
